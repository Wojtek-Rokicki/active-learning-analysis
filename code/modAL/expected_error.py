...
def expected_error_reduction(learner: ActiveLearner, X: modALinput, loss: str = 'binary',
                             ...
                             random_tie_break: bool = False, pool_candidates_size: int = None) -> np.ndarray:
    """
    ...
    Args:
        ...
        pool_candidates_size: The size of candidates, for which expected error is computed.
            It should be the size of several hundreds samples (ex. 250) for good estimation.
            If None, then EER is run on the whole Pool.
    ...
    assert len(X) > 0, 'Found array with 0 sample(s), while a minimum of 1 is required'
