...
def data_vstack(blocks: Sequence[modALinput]) -> modALinput:
    ...
    if any([sp.issparse(b) for b in blocks]):
        ...
        return blocks[0].append(list(map(lambda x: pd.DataFrame(x.reshape(1, -1), columns=list(blocks[0])), blocks[1:])))
    elif isinstance(blocks[0], pd.Series):
        return blocks[0].append(list(map(lambda x: pd.Series(x), blocks[1:])))
